...
1. Mysql Grid lets you run a mysql server instance with as many databases as you need.

    $ smartmachine grids mysql up
    $ smartmachine grids mysql down

2. Elasticsearch Grid lets you run elasticsearch server instance with data persistance.

    $ smartmachine grids elasticsearch install
    $ smartmachine grids elasticsearch up
    $ smartmachine grids elasticsearch down

4. Nginx Grid lets you run a nginx web server equipped with https encryption using letsencrypt.

    $ smartmachine grids nginx up
    $ smartmachine grids nginx down

5. Prereceiver Grid lets you push rails apps to your server without any downtime using *git push production master*

    $ smartmachine grids prereceiver install
    $ smartmachine grids prereceiver up
    $ smartmachine grids prereceiver down

6. Scheduler Grid lets you schedule various services like database backups, etc.
