...
=== Nginx Grid
    Lets you run a nginx web server equipped with https encryption using letsencrypt.
        
        $ smartmachine grids nginx up # To start the grid
        $ smartmachine grids nginx down # To stop the grid
...
=== Prereceiver Grid
   Lets you push rails apps to your server without any additional configuration or downtime using <b>git push production master</b>.
    ...
    $ smartmachine grids prereceiver install # To install the grid
    $ smartmachine grids prereceiver up # To start the grid
    $ smartmachine grids prereceiver down # To stop the grid

=== Mysql Grid
Lets you run a mysql server instance with as many databases as you need.

    $ smartmachine grids mysql up # To start the grid
    $ smartmachine grids mysql down # To stop the grid

=== Elasticsearch Grid

   Lets you run elasticsearch server instance with data persistance.

    $ smartmachine grids elasticsearch install # To install the grid
    $ smartmachine grids elasticsearch up # To start the grid
    $ smartmachine grids elasticsearch down # To stop the grid

=== Scheduler Grid

   Lets you schedule various services like database backups, etc.

    $ smartmachine grids scheduler install # To install the grid
    $ smartmachine grids scheduler up # To start the grid
    $ smartmachine grids scheduler down # To stop the grid
